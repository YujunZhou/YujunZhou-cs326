...
class IPSLSTM(nn.Module):
    def __init__(self):
        super(IPSLSTM, self).__init__()
        dropout_rate = 0.5
        input_size = 70
        hidden_size = 70
        n_labels = 3
...
class GNN(nn.Module):
    def __init__(self):
        ...
        self.hidden_size = 70
        self.hidden_channels = 70
        self.num_classes = 3
        self.num_new_features = 10
...
class GNNtest(GNN):
    def __init__(self, model_name, FSGS=True):
        ...
        self.x_0 = pickle.load(open('./gnn/IPS' + model_name + '.nodes.pickle', 'rb')).cuda()
...
class GNNadv(GNN):
    ...
    def forward(self, x, y, adv=False, fixedge=False):
        ...
        if adv or fixedge:
            ...
            norm = torch.norm(x[:4101], 2, 1).reshape(1, -1)
            a = a / norm / norm.t()
            a = a * label_mat
            top5 = a.topk(6, dim=1)[1][:, 1:]
            A = torch.eye(len(x[:4101]))[top5].sum(dim=1)
            A = A + A.t()
            edges = torch.cat(torch.where(A > 0), dim=0).reshape(2, -1).cuda()
            self.edge_0 = edges
